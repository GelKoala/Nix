...
let
  cfg = config.modules.system.hermes-agent;
in
{
  imports = [ inputs.hermes-agent.nixosModules.default ];

  options.modules.system.hermes-agent.enable =
    lib.mkEnableOption "hermes-agent (Nous Research)";
  ...
  config = lib.mkIf cfg.enable {
    ...
    services.hermes-agent = {
      ...
      container.hostUsers = [ UserConfig.username ];
      ...
      documents."USER.md" = ./_hermes-agent/USER.md;
    ...
    security.sudo.extraRules = [{
      ...
      commands = [{
        command = "/etc/profiles/per-user/${UserConfig.username}/bin/docker";
        options = [ "NOPASSWD" ];
      }];
    }];
  };
